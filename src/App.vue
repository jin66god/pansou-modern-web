...
import SearchConfig from '@/components/SearchConfigModern.vue';
...
// 原始搜索结果（未筛选）
const rawSearchResults = reactive<{
  total: number;
  mergedResults: MergedResults;
}>({
  total: 0,
...
const updateSearchResults = (response: SearchResponse) => {
  ...
  // 保存原始结果
  rawSearchResults.total = response.total || 0;
  ...
  if (response.merged_by_type) {
    ...
    rawSearchResults.mergedResults = results;
  } else {
    console.warn('搜索结果中没有merged_by_type字段');
    rawSearchResults.mergedResults = {};
  }
  ...
  <div class="app-shell min-h-screen bg-background text-foreground transition-colors duration-300 flex flex-col">
    ...
    >
      ...
      <div v-if="currentPage === 'search'" class="search-page">
        ...
        <!-- 搜索结果区域：左侧筛选 + 右侧结果 -->
        <div v-if="hasSearched || loading" class="search-results-container">
          <!-- 左侧筛选栏 -->
